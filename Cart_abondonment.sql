...
-- 3. Abandonment Rate by Device Type
WITH Stats AS (
    SELECT 
      DeviceType,
      COUNT(*) AS total_sessions,
      SUM(AbandonmentReason IS NOT NULL) AS abandoned
    FROM ecommerce_cart_abandonment
    GROUP BY DeviceType
)
SELECT 
  ...
  total_sessions,
  abandoned,
  ROUND(abandoned * 100.0 / total_sessions, 2) AS abandon_rate_percent
FROM Stats
ORDER BY abandon_rate_percent DESC;

-- 4. Abandonment by OS and Browser
WITH Stats AS (
    SELECT 
      OperatingSystem,
      BrowserType,
      ...
      SUM(AbandonmentReason IS NOT NULL) AS abandoned
    FROM ecommerce_cart_abandonment
    GROUP BY OperatingSystem, BrowserType
)
SELECT 
  OperatingSystem,
  BrowserType,
  ...
  ROUND(abandoned * 100.0 / total_sessions, 2) AS abandon_rate_percent
FROM Stats
ORDER BY abandon_rate_percent DESC, total_sessions DESC;

-- 5. Do Coupons Reduce Abandonment?
WITH Stats AS (
    SELECT 
      HasCouponApplied,
      COUNT(*) AS total_sessions,
